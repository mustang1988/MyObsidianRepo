...
```dataviewjs
const raw = await dv.view('Item/Quartz/Regular/Admonition', { quartz: dv.current().List[192], options: { raw: true } });
dv.span(raw);
```
^hunluanzhiren

```dataviewjs
const raw = await dv.view('Item/Quartz/Regular/Admonition', { quartz: dv.current().List[193], options: { raw: true } });
dv.span(raw);
```
^yinyanling

```dataviewjs
const raw = await dv.view('Item/Quartz/Regular/Admonition', { quartz: dv.current().List[194], options: { raw: true } });
dv.span(raw);
```
^huanyaoya

```dataviewjs
const raw = await dv.view('Item/Quartz/Regular/Admonition', { quartz: dv.current().List[195], options: { raw: true } });
dv.span(raw);
```
^shengwangzhu
...
dv.header(2, "行内渲染");
// 行内渲染
dv.list(await Promise.all(dv.current().List.map(q => dv.blockLink(dv.current().file.path, q.ID)).map(q => dv.view('Item/Quartz/Regular/InLine', { link: q, options: { raw: true } }))));

dv.header(2, "带数量行内渲染");
// 带数量行内渲染
dv.list(await Promise.all(dv.current().List.map(q => dv.blockLink(dv.current().file.path, q.ID)).map(q => dv.view('Item/Quartz/Regular/Count', { link: q, options: { raw: true, count: 2 } }))));
```