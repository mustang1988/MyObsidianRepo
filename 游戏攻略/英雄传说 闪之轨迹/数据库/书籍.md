...
---
```dataviewjs
dv.span(await dv.view('Item/Book/Admonition', { book: dv.current().List[0], options:{ raw:true, collapse: "open" } }));
```
^diguoshibao1

```dataviewjs
dv.span(await dv.view('Item/Book/Admonition', { book: dv.current().List[1], options:{ raw:true, collapse: "open" } }));
```
^diguoshibao2

```dataviewjs
dv.span(await dv.view('Item/Book/Admonition', { book: dv.current().List[2], options:{ raw:true, collapse: "open" } }));
```
^diguoshibao3

```dataviewjs
dv.span(await dv.view('Item/Book/Admonition', { book: dv.current().List[3], options:{ raw:true, collapse: "open" } }));
```
^diguoshibao4

```dataviewjs
dv.span(await dv.view('Item/Book/Admonition', { book: dv.current().List[4], options:{ raw:true, collapse: "open" } }));
```
^diguoshibao5

```dataviewjs
dv.span(await dv.view('Item/Book/Admonition', { book: dv.current().List[5], options:{ raw:true, collapse: "open" } }));
```
^diguoshibao6

```dataviewjs
dv.span(await dv.view('Item/Book/Admonition', { book: dv.current().List[6], options:{ raw:true, collapse: "open" } }));
```
^diguoshibao7

```dataviewjs
dv.span(await dv.view('Item/Book/Admonition', { book: dv.current().List[7], options:{ raw:true, collapse: "open" } }));
```
^diguoshibao8

```dataviewjs
dv.span(await dv.view('Item/Book/Admonition', { book: dv.current().List[8], options:{ raw:true, collapse: "open" } }));
```
^diguoshibao9

```dataviewjs
dv.span(await dv.view('Item/Book/Admonition', { book: dv.current().List[9], options:{ raw:true, collapse: "open" } }));
```
^diguoshibao10

```dataviewjs
dv.span(await dv.view('Item/Book/Admonition', { book: dv.current().List[10], options:{ raw:true, collapse: "open" } }));
```
^diguoshibao11

```dataviewjs
dv.span(await dv.view('Item/Book/Admonition', { book: dv.current().List[11], options:{ raw:true, collapse: "open" } }));
```
^hongyaoshi1

```dataviewjs
dv.span(await dv.view('Item/Book/Admonition', { book: dv.current().List[12], options:{ raw:true, collapse: "open" } }));
```
^hongyaoshi2

```dataviewjs
dv.span(await dv.view('Item/Book/Admonition', { book: dv.current().List[13], options:{ raw:true, collapse: "open" } }));
```
^hongyaoshi3

```dataviewjs
dv.span(await dv.view('Item/Book/Admonition', { book: dv.current().List[14], options:{ raw:true, collapse: "open" } }));
```
^hongyaoshi4

```dataviewjs
dv.span(await dv.view('Item/Book/Admonition', { book: dv.current().List[15], options:{ raw:true, collapse: "open" } }));
```
^hongyaoshi5

```dataviewjs
dv.span(await dv.view('Item/Book/Admonition', { book: dv.current().List[16], options:{ raw:true, collapse: "open" } }));
```
^hongyaoshi6

```dataviewjs
dv.span(await dv.view('Item/Book/Admonition', { book: dv.current().List[17], options:{ raw:true, collapse: "open" } }));
```
^hongyaoshi7

```dataviewjs
dv.span(await dv.view('Item/Book/Admonition', { book: dv.current().List[18], options:{ raw:true, collapse: "open" } }));
```
^hongyaoshi8

```dataviewjs
dv.span(await dv.view('Item/Book/Admonition', { book: dv.current().List[19], options:{ raw:true, collapse: "open" } }));
```
^hongyaoshi9

```dataviewjs
dv.span(await dv.view('Item/Book/Admonition', { book: dv.current().List[20], options:{ raw:true, collapse: "open" } }));
```
^hongyaoshi10

```dataviewjs
dv.span(await dv.view('Item/Book/Admonition', { book: dv.current().List[21], options:{ raw:true, collapse: "open" } }));
```
^hongyaoshi11

```dataviewjs
dv.span(await dv.view('Item/Book/Admonition', { book: dv.current().List[22], options:{ raw:true, collapse: "open" } }));
```
^hongyuedeluosai1

```dataviewjs
dv.span(await dv.view('Item/Book/Admonition', { book: dv.current().List[23], options:{ raw:true, collapse: "open" } }));
```
^hongyuedeluosai2

```dataviewjs
dv.span(await dv.view('Item/Book/Admonition', { book: dv.current().List[24], options:{ raw:true, collapse: "open" } }));
```
^hongyuedeluosai3

```dataviewjs
dv.span(await dv.view('Item/Book/Admonition', { book: dv.current().List[25], options:{ raw:true, collapse: "open" } }));
```
^hongyuedeluosai4

```dataviewjs
dv.span(await dv.view('Item/Book/Admonition', { book: dv.current().List[26], options:{ raw:true, collapse: "open" } }));
```
^hongyuedeluosai5

```dataviewjs
dv.span(await dv.view('Item/Book/Admonition', { book: dv.current().List[27], options:{ raw:true, collapse: "open" } }));
```
^hongyuedeluosai6

```dataviewjs
dv.span(await dv.view('Item/Book/Admonition', { book: dv.current().List[28], options:{ raw:true, collapse: "open" } }));
```
^hongyuedeluosai7

```dataviewjs
dv.span(await dv.view('Item/Book/Admonition', { book: dv.current().List[29], options:{ raw:true, collapse: "open" } }));
```
^hongyuedeluosai8

```dataviewjs
dv.span(await dv.view('Item/Book/Admonition', { book: dv.current().List[30], options:{ raw:true, collapse: "open" } }));
```
^hongyuedeluosai9

```dataviewjs
dv.span(await dv.view('Item/Book/Admonition', { book: dv.current().List[31], options:{ raw:true, collapse: "open" } }));
```
^hongyuedeluosai10

```dataviewjs
dv.span(await dv.view('Item/Book/Admonition', { book: dv.current().List[32], options:{ raw:true, collapse: "open" } }));
```
^hongyuedeluosai11

```dataviewjs
dv.span(await dv.view('Item/Book/Admonition', { book: dv.current().List[33], options:{ raw:true, collapse: "open" } }));
```
^hongyuedeluosai12

```dataviewjs
dv.span(await dv.view('Item/Book/Admonition', { book: dv.current().List[34], options:{ raw:true, collapse: "open" } }));
```
^hongyuedeluosai13

```dataviewjs
dv.span(await dv.view('Item/Book/Admonition', { book: dv.current().List[35], options:{ raw:true, collapse: "open" } }));
```
^hongyuedeluosai14

```dataviewjs
dv.span(await dv.view('Item/Book/Admonition', { book: dv.current().List[36], options:{ raw:true, collapse: "open" } }));
```
^diguotielushi

```dataviewjs
dv.span(await dv.view('Item/Book/Admonition', { book: dv.current().List[37], options:{ raw:true, collapse: "open" } }));
```
^diguodechuanshuoshang

```dataviewjs
dv.span(await dv.view('Item/Book/Admonition', { book: dv.current().List[38], options:{ raw:true, collapse: "open" } }));
```
^jindaiyundong

```dataviewjs
dv.span(await dv.view('Item/Book/Admonition', { book: dv.current().List[39], options:{ raw:true, collapse: "open" } }));
```
^diguodechuanshuozhong

```dataviewjs
dv.span(await dv.view('Item/Book/Admonition', { book: dv.current().List[40], options:{ raw:true, collapse: "open" } }));
```
^baozhiyuguangbo

```dataviewjs
dv.span(await dv.view('Item/Book/Admonition', { book: dv.current().List[41], options:{ raw:true, collapse: "open" } }));
```
^diguodechuanshuoxia

```dataviewjs
dv.span(await dv.view('Item/Book/Admonition', { book: dv.current().List[42], options:{ raw:true, collapse: "open" } }));
```
^rfjituandelishi

---
```dataviewjs
const books = await Promise.all(dv.current().List.map(book => dv.blockLink(dv.current().file.path, book.ID)).map(book => dv.view('Item/Book/InLine', { link: book, options: { raw: true } })))
dv.list(books)
```