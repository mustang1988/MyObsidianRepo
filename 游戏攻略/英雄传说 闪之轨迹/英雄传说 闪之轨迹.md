...
dv.table(
   ...
   dv.pages('#英雄传说/闪之轨迹/笔记/钓鱼 and -"模板"')
     .map(fish=>[
	   ...
	   fish.Rods,
	   fish.Positions,
	   fish.Bonus.map(b => dv.paragraph((Array.isArray(b.Icon) ? b.Icon.join('') : b.Icon) + b.Name + (b.Num > 1 ? 'x'+b.Num: '')))
	 ])
);
~~~
```
...
dv.table(
   ...
   .map(box => [
     ...
     box.Chapter,
     box.Position,
     dv.paragraph((Array.isArray(box.Bonus.Icon) ? box.Bonus.Icon.join('') : box.Bonus.Icon) + box.Bonus.Name + (box.Bonus.Num > 1 ? 'x'+box.Bonus.Num: ''))
   ])
);
~~~
```
