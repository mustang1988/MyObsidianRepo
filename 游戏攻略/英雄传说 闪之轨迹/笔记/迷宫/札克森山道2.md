...
Name: "札克森山道2"
Chapter:
- "[[游戏攻略/英雄传说 闪之轨迹/攻略/第六章 黑与银 ~钢都动乱~#^ch-6-9-25-quest-02|第六章9/25]]"
FishingPorts:
- "东北面T字形路口, 瀑布方向"
---
# `$=dv.current().Name;`
...
```leaflet
id: "札克森山道2"
image: "![[图片/游戏攻略/英雄传说/英雄传说 闪之轨迹/笔记/迷宫/札克森山道2.jpg]]"
height: 500px
minZoom: 1
maxZoom: 10
defaultZoom: 9
...
const notes = dv.pages('#英雄传说/闪之轨迹/笔记/战斗 and -"模板"')
	.filter(i => i.Positions
				.map(position => dv.page(position.path).file.name)
				.includes(dv.current().name)
	)
const views = await Promise.all(notes.map(n => dv.view('Item', { items: n.Bonus, options: { inline: false, raw: true } })));
dv.table(
	["名称", "掉落"],
	notes.map((m, index) => [
		m.file.link,
