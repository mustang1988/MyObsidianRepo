...
const GetUMaterial = (link, withName) => {
  ...
  return {item, link:dv.blockLink(path, subpath, false, withName ? item.Name : " ")}
}
...
const GetUMaterialIcon = async(size) => {
  return dv.view('Icons/Icon', { key: U_MATERIAL_ICON_KEY, options: { raw: true } })
  .then(icon => {
    if(icon === null){
      return '';
    }
...
DEBUG && console.debug("[U物质InLine渲染][Views/Item/UMaterial/InLine/view.js][Input]:\n", { link, options });
